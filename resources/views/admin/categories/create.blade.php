...
<div class="space-y-6 max-w-2xl">
    ...
    <div class="flex items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-brand-charcoal">افزودن دسته‌بندی جدید</h1>
            <p class="text-sm text-gray-500 mt-1">ایجاد دسته‌بندی برای محصولات</p>
        </div>
    ...
    <form method="POST" action="{{ route('admin.categories.store') }}" class="bg-white rounded-2xl shadow-sm border border-gray-100/80 p-6 space-y-5">
        @csrf
        ...
        <div>
            ...
            <input type="text" id="name" name="name" value="{{ old('name') }}" required class="w-full bg-gray-50 rounded-lg px-4 py-2.5 text-sm border border-gray-200 focus:ring-2 focus:ring-brand-red/30 focus:bg-white focus:border-brand-red/50 transition-all" placeholder="مثال: سیستم ترمز">
        ...
        <div>
            ...
            <input type="text" id="slug" name="slug" value="{{ old('slug') }}" required dir="ltr" class="w-full bg-gray-50 rounded-lg px-4 py-2.5 text-sm border border-gray-200 focus:ring-2 focus:ring-brand-red/30 focus:bg-white focus:border-brand-red/50 transition-all font-num" placeholder="brake-system">
        ...
        <div>
            ...
            <input type="text" id="icon" name="icon" value="{{ old('icon') }}" dir="ltr" class="w-full bg-gray-50 rounded-lg px-4 py-2.5 text-sm border border-gray-200 focus:ring-2 focus:ring-brand-red/30 focus:bg-white focus:border-brand-red/50 transition-all font-num" placeholder="disc">
            <p class="text-xs text-gray-400 mt-1.5">نام آیکون از کتابخانه <a href="https://lucide.dev/icons/" target="_blank" class="text-brand-red hover:underline">Lucide</a>، مانند: disc، settings، wrench</p>
