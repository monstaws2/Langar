...
@section('content')
    <div class="max-w-xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <form action="{{ route('admin.motorcycle-models.update', $motorcycleModel) }}" method="POST" class="bg-white rounded-lg shadow overflow-hidden">
            @csrf
            @method('PUT')
            <div class="px-6 py-6">
                <h2 class="text-xl font-medium text-brand-charcoal mb-6">ویرایش مدل موتورسیکلت: {{ $motorcycleModel->name }}</h2>
                ...
                <div class="grid grid-cols-1 gap-6 mb-6">
                    <div>
                        ...
                        <select name="brand_id" id="brand_id"
                            ...
                            @foreach($brands as $id => $name)
                                <option value="{{ $id }}" {{ old('brand_id', $motorcycleModel->brand_id) == $id ? 'selected' : '' }}>
                                    {{ $name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $motorcycleModel->name) }}"
                               class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-red focus:border-transparent"
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="year_from" id="year_from" value="{{ old('year_from', $motorcycleModel->year_from) }}"
                                   class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-red focus:border-transparent"
                        ...
                        <div>
                            ...
                            <input type="number" name="year_to" id="year_to" value="{{ old('year_to', $motorcycleModel->year_to) }}"
                                   class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-red focus:border-transparent"
                                   min="1900" max="{{ date('Y') + 2 }}">
                            @error('year_to')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div class="flex items-center">
                        <div class="flex items-center mb-0">
                            <input type="checkbox" name="is_active" id="is_active" {{ old('is_active', $motorcycleModel->is_active) ? 'checked' : '' }}
